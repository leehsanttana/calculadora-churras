...
export default function CalculadoraForm() {
  ...
  const categoriasCarne: CategoriaSelecao[] = CATEGORIAS.filter((cat) =>
    ...
    selecionados: cortes,
    onToggle: (id) => toggleEm(setCortes, id),
  }));
  ...
  const categoriasExtras: CategoriaSelecao[] = [
    {
      ...
      onToggle: (id) => toggleEm(setAcompanhamentos, id),
    },
    {
      id: "sobremesa",
      nome: "Sobremesas",
      ...
      onToggle: (id) => toggleEm(setSobremesas, id),
    },
    {
      id: "bebida",
      nome: "Bebidas",
      ...
      onToggle: (id) => toggleEm(setBebidas, id),
    },
  ];
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-8">
      ...
      {etapa === 0 && (
        <EtapaSobre entrada={entrada} set={set} perfis={PERFIS} duracoes={DURACOES} />
      )}
      {etapa === 1 && (
        <EtapaCategorias selecionadas={categorias} onToggle={toggleCategoria} />
      )}
      {etapa === 2 && (
        <PassoSelecao
...
/** Barra de abas (pílulas) reutilizável. */
function TabPills({
  abas,
  ativa,
  onChange,
}: {
  abas: { id: string; nome: string; emoji: string; count: number }[];
